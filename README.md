# mcp-jp

日本SMB向け MCP サーバー集

## コネクタ一覧

| ディレクトリ | サービス | ステータス |
|-------------|---------|-----------|
| `lstep/` | Lステップ（LINE MA） | 開発中（API全19エンドポイント実装済み） |
| `freee/` | freee会計 | ~~開発中~~ → **公式MCPあり**（2026年3月公開、270 API）|
| `mfcloud/` | マネーフォワードクラウド | ~~開発中~~ → **公式MCPあり**（2026年3月全プラン開放）|
| `kintone/` | kintone | ~~開発中~~ → **公式MCPあり**（2026年5月公式QSG公開）|
| `kingtime/` | KING OF TIME（勤怠管理） | 開発中（5ツール実装済み） |
| `smarthr/` | SmartHR（人事・労務管理） | 開発中（5ツール実装済み） |
| `kaonavi/` | カオナビ（タレントマネジメント） | 開発中（5ツール実装済み） |
| `mazrica/` | Mazrica Sales（SFA・営業支援） | 開発中（5ツール実装済み） |
| `invox/` | invox（受取請求書・仕訳エクスポート） | 開発中（5ツール実装済み） |
| `smaregi/` | スマレジ（POSレジ・店舗管理） | 開発中（5ツール実装済み） |
| `chatwork/` | Chatwork（ビジネスチャット） | 開発中（5ツール実装済み） |
| `backlog/` | Backlog（プロジェクト管理・課題追跡） | 開発中（5ツール実装済み） |
| `garoon/` | Garoon（サイボウズ グループウェア） | 開発中（5ツール実装済み） |
| `jobcan-workflow/` | ジョブカン経費精算/ワークフロー | 開発中（5ツール実装済み） |
| `base-ec/` | BASE（ECプラットフォーム） | 開発中（5ツール実装済み） |
| `sansan/` | Sansan（名刺管理・人脈管理） | 開発中（5ツール実装済み） |
| `esa/` | esa（チームナレッジ共有） | 開発中（5ツール実装済み） |
| `zendesk/` | Zendesk（カスタマーサポート） | 開発中（5ツール実装済み） |
| `hubspot/` | HubSpot CRM（営業・マーケティング） | 開発中（5ツール実装済み） |
| `notion/` | Notion（ナレッジ管理・プロジェクト管理） | 開発中（5ツール実装済み） |
| `stripe/` | Stripe（決済・請求管理） | 開発中（5ツール実装済み） |
| `sendgrid/` | SendGrid（メール送信・マーケティング） | 開発中（5ツール実装済み） |
| `twilio/` | Twilio（SMS・音声通話） | 開発中（5ツール実装済み） |
| `airtable/` | Airtable（スプレッドシート型DB） | 開発中（5ツール実装済み） |
| `gitlab/` | GitLab（ソースコード管理・CI/CD） | 開発中（5ツール実装済み） |
| `asana/` | Asana（プロジェクト管理・タスク管理） | 開発中（5ツール実装済み） |
| `jira/` | Jira（プロジェクト管理・課題追跡） | 開発中（5ツール実装済み） |
| `freshdesk/` | Freshdesk（カスタマーサポート・ヘルプデスク） | 開発中（5ツール実装済み） |
| `pipedrive/` | Pipedrive（CRM・営業パイプライン管理） | 開発中（5ツール実装済み） |
| `shopify/` | Shopify（ECプラットフォーム） | 開発中（5ツール実装済み） |
| `mailchimp/` | Mailchimp（メールマーケティング） | 開発中（5ツール実装済み） |
| `confluence/` | Confluence（ナレッジ管理・ドキュメント共有） | 開発中（5ツール実装済み） |
| `intercom/` | Intercom（カスタマーメッセージング・CRM） | 開発中（5ツール実装済み） |
| `square/` | Square（決済・店舗管理） | 開発中（5ツール実装済み） |
| `box/` | Box（クラウドストレージ・ドキュメント管理） | 開発中（5ツール実装済み） |
| `zoom/` | Zoom（ビデオ会議・ウェビナー管理） | 開発中（5ツール実装済み） |
| `dropbox/` | Dropbox（クラウドストレージ・ファイル共有） | 開発中（5ツール実装済み） |
| `pagerduty/` | PagerDuty（インシデント管理・オンコール管理） | 開発中（5ツール実装済み） |
| `datadog/` | Datadog（クラウド監視・モニタリング） | 開発中（5ツール実装済み） |
| `typeform/` | Typeform（フォーム作成・アンケート） | 開発中（5ツール実装済み） |
| `clickup/` | ClickUp（プロジェクト管理・タスク管理） | 開発中（5ツール実装済み） |
| `toggl/` | Toggl Track（時間計測・工数管理） | 開発中（5ツール実装済み） |
| `calendly/` | Calendly（スケジュール調整・予約管理） | 開発中（5ツール実装済み） |
| `harvest/` | Harvest（時間計測・請求書・プロジェクト管理） | 開発中（5ツール実装済み） |

## 次の候補

| サービス | カテゴリ | API状況 | 優先度 |
|---------|---------|--------|-------|
| CYDAS | タレントマネジメント | 要申し込み（developer.cydas.com） | 高（アカウント取得後） |
| LetterSeal | 郵送 | 調査中 | 中 |

## 思想

- ベンダーがMCPを出していればそれを使う
- 日本SMBローカルのSaaSで、ベンダーが作らないものを自分たちが作る
- Claude等のAIエージェントから日本のSaaSを操作できるようにする

## 使い方

各ディレクトリのREADMEを参照。
